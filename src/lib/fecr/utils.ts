export function round10(num: number, expr: number) {
  let value: number | string[] = num;
  let exp = expr;
  const type = "round";
...
export function zfill(number: string | number, width: number) {
  const numberOutput = Math.abs(Number(number)); /* Valor absoluto del número */
...
export function parseVouchers(xml2js: any) {
  let doc;
  let documentType;
  ...
  const issuer = {
    ...
    tradename: doc.Emisor[0].NombreComercial
      ? doc.Emisor[0].NombreComercial[0]
      : null,
  ...
  const items: any[] = [];
  doc.DetalleServicio[0].LineaDetalle.forEach((item: any) => {
    const tmpItem: any = {
      line: item.NumeroLinea[0],
      commercialCode: item.CodigoComercial
        ? item.CodigoComercial[0].Codigo[0]
        : null,
  ...
  if (doc.Receptor) {
    const receiver: any = {
      ...
      tradename: doc.Receptor[0].NombreComercial
        ? doc.Receptor[0].NombreComercial[0]
        : null,
      email: doc.Receptor[0].CorreoElectronico
        ? doc.Receptor[0].CorreoElectronico[0]
        : null,
      phone: doc.Receptor[0].Telefono
        ? doc.Receptor[0].Telefono[0].NumTelefono[0]
        : null,
    };
  ...
  if (doc.InformacionReferencia) {
    ...
    doc.InformacionReferencia.forEach((reference: any) => {
      references.push({
